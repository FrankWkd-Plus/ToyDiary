...
WAV into /home/talk2/ring/input for STT.
"""
...
async def connect_ring(args: RecorderArgs) -> tuple[Any, str]:
    print("[record] checking ring bluetooth connectivity...")
    devices = await sdk.scan_rings(address=args.address, timeout_s=args.scan_timeout_s)
    if not devices:
        raise RuntimeError("没有扫描到 Ring Sound 设备；请确认戒指已开机、蓝牙已连接")
    device = devices[0]
    label = device.name or device.address
    print(f"[record] bluetooth ok: found {len(devices)} device(s), using {label}")
...
async def wait_and_save(args: RecorderArgs) -> Path:
    ...
    try:
        print(f"[record] connected: {label}")
